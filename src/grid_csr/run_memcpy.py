...
""" Compute A*B using a 2-by-2 PE rectangle
   ...
   A functions spmm_csr_f32 is used to compute C=A*B using the CSR grid format.
   This function is imported as a module via spmm_csr.csl.
   The arrays A_val, A_col_idx, A_row_ptr, B, C are passed into the function as pointers.
...
#  if not, must redo the routing
  ...
  A_col_idx = np.genfromtxt(A_colidx_file, delimiter=",", dtype=np.float32)
  A_row_ptr = np.genfromtxt(A_rowptr_file, delimiter=",", dtype=np.float32)
  ...
  simulator.memcpy_h2d(symbol_A_col_idx, data, False, px, py, w, h, l, 0, False)
  (px, py, w, h, l, data) = runtime_utils.convert_input_tensor(iportmap_A_row_ptr, A_row_ptr)
  simulator.memcpy_h2d(symbol_A_row_ptr, data, False, px, py, w, h, l, 0, False)
